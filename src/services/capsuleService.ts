...
import { sendCapsuleUnlockNotification } from './notificationService';
 ...
 */
async function deliverUnlockEvent(
  io: SocketIOServer,
  recipientId: string,
  payload: CapsuleUnlockPayload
): Promise<void> {
  ...
  try {
    await sendCapsuleUnlockNotification(recipientId, payload.messageId);
  } catch (err) {
 ...
 * @param io - The shared Socket.io server instance
 */
export async function runCapsuleUnlockScheduler(
  io: SocketIOServer
): Promise<void> {
  const now = new Date();
  ...
  for (const msg of messages) {
    ...
    const payload: CapsuleUnlockPayload = { messageId, conversationId };
    ...
    try {
      ...
      console.error(
        `[CapsuleService] Error processing unlock for message ${messageId}:`,
        err
      );
    }
  }
 ...
 */
export async function handleConditionMet(
  io: SocketIOServer,
  messageId: string,
  callerId: string
): Promise<Record<string, unknown>> {
  if (!Types.ObjectId.isValid(messageId)) {
    const err = new Error('Invalid message ID.') as Error & {
      statusCode: number;
      code: string;
    };
    err.statusCode = 400;
  ...
  if (!message) {
    const err = new Error('Message not found.') as Error & {
      statusCode: number;
      code: string;
    };
    err.statusCode = 404;
  ...
  if (!message.capsule?.enabled || message.capsule.type !== 'condition') {
    const err = new Error(
      'This message is not a condition-based capsule.'
  ...
  if (message.capsule.status === 'unlocked') {
    const err = new Error('This capsule has already been unlocked.') as Error & {
      statusCode: number;
      code: string;
    };
    err.statusCode = 409;
  ...
  if (callerId === senderId) {
    const err = new Error(
      'Only the recipient can signal that a condition has been met.'
    ) as Error & { statusCode: number; code: string };
    err.statusCode = 403;
  ...
  const payload: CapsuleUnlockPayload = {
...
export async function drainPendingEvents(
  ...
  const events = await PendingEvent.find({
    recipientId: new Types.ObjectId(recipientId),
  })
    .sort({ createdAt: 1 })
    .lean();
  ...
  await PendingEvent.deleteMany({ _id: { $in: deliveredIds } });
}
