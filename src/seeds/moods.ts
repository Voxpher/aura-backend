 ...
 */
export async function getMoodById(id: string): Promise<IMood | null> {
  return Mood.findById(id).exec();
}

/**
 * Returns all Mood documents in the collection.
 */
export async function getAllMoods(): Promise<IMood[]> {
  return Mood.find().exec();
}
